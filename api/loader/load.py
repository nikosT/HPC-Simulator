...
⊙ Coloads: {list(self.coscheduled_timelogs.keys())}"""
    ...
    def get_avg_speedup(self, co_load: T, policy='cmp') -> float:
        """Return the average speedup when co-scheduled with a load

        ⟡ co_load: the co_scheduled load; Load or str
        """
        return (self.get_avg_time(policy=policy) / self.get_avg_time(str(co_load)))

    def get_med_speedup(self, co_load: T, policy='cmp') -> float:
        """Return the median speedup when co-scheduled with a load

        ⟡ co_load: the co_scheduled load; Load or str
        """
        return (self.get_med_time(policy=policy) /self.get_med_time(str(co_load)))
